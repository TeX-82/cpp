...
etc.
*/
constexpr auto random_reading = true; /*should we skip around in the file?*/
...
#include "Types_in_the_outer_block.cpp"
#include "Globals_in_the_outer_block.cpp"

web_DEFINE(Set_initial_values);
#define Set_initial_values static web::function_static_initialization web_function(x) {Set_initial_values_functions[__LINE__]

static void initialize() /*this procedure gets things started properly*/
{
	print_ln(banner);
	Set_initial_values_();
}
...
*/
constexpr auto max_widths = 10000; /*maximum number of different characters among all fonts*/
constexpr auto terminal_line_length = 150; /*maximum number of characters input in a single
  line of input from the terminal*/
constexpr auto stack_size = 100; /*\.{DVI} files shouldn't |push| beyond this depth*/
constexpr auto line_length = 79; /*bracketed lines of output will be at most this long*/
constexpr auto max_fonts = 100; /*maximum number of distinct fonts per \.{DVI} file*/
constexpr auto name_size = 1000; /*total length of all font file names*/

/*
@ Here are some macros for common programming idioms.
*/
#define do_nothing {} /*empty statement*/ 
template<typename T> inline void incr(T& i) { i++; } // increase a variable by unity
template<typename T> inline void decr(T& i) { i--; } // decrease a variable by unity
...
template<typename ...Args> void abort(Args... args) {
	print(" ", args...); jump_out();
}
template<typename ...Args> void bad_dvi(Args... args) { abort("Bad DVI file: ", args..., '!'); }
//@.Bad DVI file@>
...
*/
using ASCII_code = subrange<' ', '~', char>; /*a subrange of the integers*/
...
*/
#include "Types_in_the_outer_block.cpp"
constexpr text_char first_text_char = 0; /*ordinal number of the smallest element of |text_char|*/
constexpr text_char last_text_char = 127; /*ordinal number of the largest element of |text_char|*/
...
*/
Set_initial_values = [&]() {
	for (integer i = 0; i <= 037; i++) xchr[i] = '?';
	xchr[040] = ' ';
	xchr[041] = '!';
	xchr[042] = '"';
	xchr[043] = '#';
	...
	xchr[0175] = '}';
	xchr[0176] = '~';
	for (integer i = 0177; i <= 255; i++) xchr[i] = '?';
} };
...
*/
Set_initial_values = [&]() {
	for (integer i = first_text_char; i <= last_text_char; i++) xord[chr(i)] = 040;
	for (integer i = ' '; i <= '~'; i++) xord[xchr[i]] = i; } };
...
*/
namespace DVI {
	constexpr auto set_char_0 = 0; /*typeset character 0 and move right*/
	constexpr auto set1 = 128; /*typeset a character and move right*/
	constexpr auto set_rule = 132; /*typeset a rule and move right*/
	constexpr auto put1 = 133; /*typeset a character*/
	constexpr auto put_rule = 137; /*typeset a rule*/
	constexpr auto nop = 138; /*no operation*/
	constexpr auto bop = 139; /*beginning of page*/
	constexpr auto eop = 140; /*ending of page*/
	constexpr auto push = 141; /*save the current positions*/
	constexpr auto pop = 142; /*restore previous positions*/
	constexpr auto right1 = 143; /*move right*/
	constexpr auto w0 = 147; /*move right by |w|*/
	constexpr auto w1 = 148; /*move right and set |w|*/
	constexpr auto x0 = 152; /*move right by |x|*/
	constexpr auto x1 = 153; /*move right and set |x|*/
	constexpr auto down1 = 157; /*move down*/
	constexpr auto y0 = 161; /*move down by |y|*/
	constexpr auto y1 = 162; /*move down and set |y|*/
	constexpr auto z0 = 166; /*move down by |z|*/
	constexpr auto z1 = 167; /*move down and set |z|*/
	constexpr auto fnt_num_0 = 171; /*set current font to 0*/
	constexpr auto fnt1 = 235; /*set current font*/
	constexpr auto xxx1 = 239; /*extension to \.{DVI} primitives*/
	constexpr auto xxx4 = 242; /*potentially long extension to \.{DVI} primitives*/
	constexpr auto fnt_def1 = 243; /*define the meaning of a font number*/
	constexpr auto pre = 247; /*preamble*/
	constexpr auto post = 248; /*postamble beginning*/
	constexpr auto post_post = 249; /*postamble ending*/
#define undefined_commands	case 250: case 251: case 252: case 253: case 254: case 255
}
...
*/
constexpr auto id_byte = 2; /*identifies the kind of \.{DVI} files described here*/
...
*/
using eight_bits = uint8_t; /*subrange<0, 255>*/; /*unsigned one-byte quantity*/
using byte_file = packed_file<eight_bits>;
...
being read.
*/
static byte_file dvi_file; /*the stuff we are \.{DVI}typing*/
static byte_file tfm_file; /*a font metric file*/
...
@^system dependencies@>
*/
void open_dvi_file() /*prepares to read packed bytes in |dvi_file|*/
{
	reset(dvi_file, "trip.dvi", "rb");
	cur_loc = 0;
}

void open_tfm_file() /*prepares to read packed bytes in |tfm_file|*/
{
	reset(tfm_file, cur_name, "rb");
}
...
@^system dependencies@>
*/
integer get_byte() /*returns the next byte, unsigned*/
{
	eight_bits b;
	if (eof(dvi_file)) return 0;
	else {
		read(dvi_file, b); incr(cur_loc); return b;
	}
}

integer signed_byte() /*returns the next byte, signed*/
{
	eight_bits b;
...
integer get_two_bytes() /*returns the next two bytes, unsigned*/
{
	eight_bits a, b;
...
integer signed_pair() /*returns the next two bytes, signed*/
{
	eight_bits a, b;
	read(dvi_file, a); read(dvi_file, b);
	cur_loc = cur_loc + 2;
...
integer get_three_bytes() /*returns the next three bytes, unsigned*/
{
	eight_bits a, b, c;
...
integer signed_trio() /*returns the next three bytes, signed*/
{
	eight_bits a, b, c;
	read(dvi_file, a); read(dvi_file, b); read(dvi_file, c);
	cur_loc = cur_loc + 3;
...
integer signed_quad() /*returns the next four bytes, signed*/
{
	eight_bits a, b, c, d;
...
*/
#define char_width_end(X)	X]
#define char_width(X)	width[width_base[X]+char_width_end
constexpr auto invalid_width = 017777777777;
constexpr auto invalid_font = max_fonts;

integer font_num[max_fonts + 1]; /*external font numbers*/
...
integer font_check_sum[max_fonts + 1]; /*check sums*/
integer font_scaled_size[max_fonts + 1]; /*scale factors*/
integer font_design_size[max_fonts + 1]; /*design sizes*/
integer font_space[max_fonts + 1]; /*boundary between ``small''
	and ``large'' spaces*/
integer font_bc[max_fonts + 1]; /*beginning characters in fonts*/
integer font_ec[max_fonts + 1]; /*ending characters in fonts*/
integer width_base[max_fonts + 1]; /*index into |width| table*/
integer width[max_widths + 1]; /*character widths, in \.{DVI} units*/
uint8_t nf = 0; /*the number of known fonts*/
uint16_t width_ptr = 0; /*the number of known character widths*/
...
*/
void print_font(integer f) /*|f| is an internal font number*/
{
	integer k; /*index into |names|*/
...
integer tfm_design_size; /*design size found in |tfm_file|, in \.{DVI} units*/
real tfm_conv; /*\.{DVI} units per absolute \.{TFM} unit*/
...
class in_TFM_ final
{
	enum goto_ { nowhere, label9997, label9998, label9999 };
#define in_TFM_goto(l) goto_label_switch_(l, \
goto_label_case_(label9997); /* go here when the format is bad */ \
goto_label_case_(label9998); /* go here when the information cannot be loaded */ \
goto_label_case_(label9999); ) /* go here to exit */
	integer k; /*index for loops*/
	integer lh; /*length of the header data, in four-byte words*/
	...
	goto_ Read_and_convert_the_width_values_setting_up_the_in_width_table();
	void Replace_z_by_z_prime_and_compute_alpha_beta();
	goto_ Move_the_widths_from_in_width_to_width_and_append_pixel_width_values();

public: boolean operator()(integer z) { /*input \.{TFM} data or return |false|*/
	in_TFM_goto(Read_past_the_header_data_goto_9997_if_there_is_a_problem());
	in_TFM_goto(Store_characterwidth_indices_at_the_end_of_the_width_table());
	in_TFM_goto(Read_and_convert_the_width_values_setting_up_the_in_width_table());
	in_TFM_goto(Move_the_widths_from_in_width_to_width_and_append_pixel_width_values());
	width_ptr = wp; return true; goto label9999;
label9997: print_ln("---not loaded, TFM file is bad");
	//@.TFM file is bad@>
label9998: return false;
label9999: return true;
} private:

	goto_ Read_past_the_header_data_goto_9997_if_there_is_a_problem() {
		...
		if (width_ptr + font_ec[nf] - font_bc[nf] + 1 > max_widths)
		{
			...
			return goto_::label9998;
		}
		wp = width_ptr + font_ec[nf] - font_bc[nf] + 1;
		read_tfm_word(); nw = b0 * 256 + b1;
		if ((nw == 0) || (nw > 256)) return goto_::label9997;
		for (k = 1; k <= lh + 3; k++)
		{
			if (eof(tfm_file)) return goto_::label9997;
			read_tfm_word();
			if (k == 4)
				...
				else return goto_::label9997;
		} return goto_::nowhere;
	}

	goto_ Store_characterwidth_indices_at_the_end_of_the_width_table() {
		if (wp > 0) for (k = width_ptr; k <= wp - 1; k++)
		{
			read_tfm_word();
			if (b0 > nw) return goto_::label9997;
			width[k] = b0;
		} return goto_::nowhere;
	}
}; inline boolean in_TFM(integer z) { return in_TFM_()(z); }
...
*/
in_TFM_::goto_ in_TFM_::Read_and_convert_the_width_values_setting_up_the_in_width_table() {
	Replace_z_by_z_prime_and_compute_alpha_beta();
	for (k = 0; k <= nw - 1; k++)
	{
		read_tfm_word();
		in_width[k] = (((((b3 * z) / 0400) + (b2 * z)) / 0400) + (b1 * z)) / beta;
		if (b0 > 0) if (b0 < 255) return goto_::label9997;
		else in_width[k] = in_width[k] - alpha;
	}
	return goto_::nowhere;
}

void in_TFM_::Replace_z_by_z_prime_and_compute_alpha_beta() {
	{alpha = 16;
	while (z >= 040000000)
	{
...
*/
#define char_pixel_width(X)	pixel_width[width_base[X]+char_width_end
integer pixel_width[max_widths + 1]; /*actual character widths,
	in pixels*/
real conv; /*converts \.{DVI} units to pixels*/
real true_conv; /*converts unmagnified \.{DVI} units to pixels*/
integer numerator, denominator; /*stated conversion ratio*/
integer mag; /*magnification factor times 1000*/
...
@^system dependencies@>
*/
inline integer pixel_round(real X) { return round(conv * (X)); }

in_TFM_::goto_ in_TFM_::Move_the_widths_from_in_width_to_width_and_append_pixel_width_values() {
	if (in_width[0] != 0) return goto_::label9997; /*the first width should be zero*/
...
@^system dependencies@>
*/
constexpr auto errors_only = 0; /*value of |out_mode| when minimal printing occurs*/
constexpr auto terse = 1; /*value of |out_mode| for abbreviated output*/
constexpr auto mnemonics_only = 2; /*value of |out_mode| for medium-quantity output*/
constexpr auto verbose = 3; /*value of |out_mode| for detailed tracing*/
constexpr auto the_works = 4; /*|verbose|, plus check of postamble if |random_reading|*/

uint8_t out_mode; /*controls the amount of output*/
integer max_pages; /*at most this many |bop dotdot eop| pages will be printed*/
real resolution; /*pixels per inch*/
integer new_mag; /*if positive, overrides the postamble's magnification*/
...
*/
integer start_count[10]; /*count values to select starting page*/
boolean start_there[10]; /*is the |start_count| value relevant?*/
uint8_t start_vals = 0; /*the last count considered significant*/
integer count[10]; /*the count values on the current page*/

Set_initial_values = [&]() {
	out_mode = the_works; max_pages = 1000000; start_there[0] = false; } };

/*
@ Here is a simple subroutine that tests if the current page might be the
starting page.
*/
boolean start_match() /*does |count| match the starting spec?*/
{
	integer k; /*loop index*/
	boolean match; /*does everything match so far?*/
	match = true;
...
inline void format(integer v, integer w)
{
	web::format(term_out, v, w);
}
inline void format(real v, integer w, integer d)
{
	web::format(term_out, v, w, d);
}
...
@^system dependencies@>
*/
inline void update_terminal() { break_(term_out); } /*empty the terminal output buffer*/
...
integer get_integer()
{
	integer x; /*accumulates the value*/
	boolean negative; /*should the value be negated?*/
	if (buffer[buf_ptr] == '-')
	{
...
class dialog_ final
{
	enum goto_ { nowhere, label1, label2, label3, label4, label5 };
#define dialog_goto(l) goto_label_switch_(l, \
goto_label_case_(label1); goto_label_case_(label2); \
goto_label_case_(label3); goto_label_case_(label4); \
goto_label_case_(label5); )
	integer k; /*loop variable*/
public: void operator()() {
	if (!dvitype::TRIP) { rewrite(term_out); /*prepare the terminal for output*/ }
	else { rewrite(term_out, "trip.typ"); }
	writeln(term_out, banner);
	if (!dvitype::TRIP) {
	label1: dialog_goto(Determine_the_desired_out_mode());
	label2: dialog_goto(Determine_the_desired_start_count_values());
	label3: dialog_goto(Determine_the_desired_max_pages());
	label4: dialog_goto(Determine_the_desired_resolution());
	label5: dialog_goto(Determine_the_desired_new_mag());
	}
	else { /* Output level = 2
	Starting page = *.*.*.*.*.*.*.*.*.*
	...
	New magnification = 0 (this is the default) */
		...
		start_vals = 9;
		max_pages = 1000000;
		resolution = 7227.0 / 100.0;
		new_mag = 0;
	}
	Print_all_the_selected_options();
} private:
	void Print_all_the_selected_options();

	goto_ Determine_the_desired_out_mode() {
		write(term_out, "Output level (default=4, ? for help): ");
		out_mode = the_works; input_ln();
		if (buffer[0] != ' ')
			if ((buffer[0] >= '0') && (buffer[0] <= '4')) out_mode = buffer[0] - '0';
			else {
				write(term_out, "Type 4 for complete listing,");
				write(term_out, " 0 for errors and fonts only,");
				writeln(term_out, " 1 or 2 or 3 for something in between.");
				return goto_::label1;
			}
		return goto_::nowhere;
	}

	goto_ Determine_the_desired_start_count_values() {
		...
		if (buffer[0] != ' ')
			do {
				...
				if ((k < 9) && (buffer[buf_ptr] == '.'))
				{
					...
					write(term_out, "Type, e.g., 1.*.-5 to specify the ");
					writeln(term_out, "first page with \\count0=1, \\count2=-5.");
					return goto_::label2;
				}
			} while (!(start_vals == k));
			return goto_::nowhere;
	...
	goto_ Determine_the_desired_max_pages() {
		...
		if (buffer[0] != ' ')
		{
			...
			if (max_pages <= 0)
			{
				writeln(term_out, "Please type a positive number.");
				return goto_::label3;
			}
		}
		return goto_::nowhere;
	...
	goto_ Determine_the_desired_resolution() {
		write(term_out, "Assumed device resolution");
		...
		if (buffer[0] != ' ')
		{
			...
			{
				incr(buf_ptr); resolution = k / (real)get_integer();
			}
			else {
				write(term_out, "Type a ratio of positive integers;");
				writeln(term_out, " (1 pixel per mm would be 254/10).");
				return goto_::label4;
			}
		}
		return goto_::nowhere;
	...
	goto_ Determine_the_desired_new_mag() {
		write(term_out, "New magnification (default=0 to keep the old one): ");
		new_mag = 0; input_ln(); buf_ptr = 0;
		if (buffer[0] != ' ')
			if ((buffer[0] >= '0') && (buffer[0] <= '9')) new_mag = get_integer();
			else {
				write(term_out, "Type a positive integer to override ");
				writeln(term_out, "the magnification in the DVI file.");
				return goto_::label5;
			}
		return goto_::nowhere;
	}
}; inline void dialog() { return dialog_()(); }

/*
@ After the dialog is over, we print the options so that the user
can see what \.{DVItype} thought was specified.
*/
void dialog_::Print_all_the_selected_options() {
	print_ln("Options selected:");
	//@.Options selected@>
	print("  Starting page = ");
	for (k = 0; k <= start_vals; k++)
	{
		if (start_there[k]) print(start_count[k]);
		else print("*");
		if (k < start_vals) print(".");
		else print_ln(" ");
	}
	print_ln("  Maximum number of pages = ", max_pages);
	print("  Output level = ", out_mode);
	switch (out_mode) {
	...
	print("  Resolution = "); format(resolution, 12, 8); print_ln(" pixels per inch");
	if (new_mag > 0) print("  New magnification factor = "), format(new_mag / 1000.0, 8, 3), print_ln("");
}
...
*/
boolean in_postamble = false; /*are we reading the postamble?*/

/*
@ The following subroutine does the necessary things when a \\{fnt\_def}
command is being processed.
*/
class define_font_ final
{
	uint8_t f;
	integer p; /*length of the area/directory spec*/
	integer n; /*length of the font name proper*/
	integer c, q, d, m; /*check sum, scaled size, design size, magnification*/
	uint8_t r; /*index into |cur_name|*/
	integer j, k; /*indices into |names|*/
	boolean mismatch; /*do names disagree?*/
	integer e;
public: void operator()(integer e_) {
	e = e_; /*|e| is an external font number*/
	if (nf == max_fonts) abort("DVItype capacity exceeded (max fonts=",
		max_fonts, ")!");
	//@.DVItype capacity exceeded...@>
	font_num[nf] = e; f = 0;
	while (font_num[f] != e) incr(f);
	Read_the_font_parameters_into_position_for_font_nf_and_print_the_font_name();
	...
	if (f == nf) Load_the_new_font_unless_there_are_problems();
	else Check_that_the_current_font_definition_matches_the_old_one();
} private:

	void Check_that_the_current_font_definition_matches_the_old_one() {
	...
	void Read_the_font_parameters_into_position_for_font_nf_and_print_the_font_name() {
		c = signed_quad(); font_check_sum[nf] = c;
		q = signed_quad(); font_scaled_size[nf] = q;
		d = signed_quad(); font_design_size[nf] = d;
		if ((q <= 0) || (d <= 0)) m = 1000;
		else m = round((1000.0 * conv * q) / (real)(true_conv * d));
		p = get_byte(); n = get_byte();
		if (font_name[nf] + n + p > name_size)
			abort("DVItype capacity exceeded (name size=", name_size, ")!");
		//@.DVItype capacity exceeded...@>
		...
		print_font(nf);
		if (!showing) if (m != 1000) print(" scaled ", m);
		//@.scaled@>
	}

	void Move_font_name_into_the_cur_name_string();
	void Load_the_new_font_unless_there_are_problems() {
		{ Move_font_name_into_the_cur_name_string();
		open_tfm_file();
		if (eof(tfm_file))
		...
		//@.TFM file can\'t be opened@>
			...
			if ((q <= 0) || (q >= 01000000000))
				print("---not loaded, bad scale (", q, ")!");
			//@.bad scale@>
			else if ((d <= 0) || (d >= 01000000000))
				print("---not loaded, bad design size (", d, ")!");
			//@.bad design size@>
			else if (in_TFM(q)) Finish_loading_the_new_font_info();
		}
		if (out_mode == errors_only) print_ln(' ');
		}
	}

	void Finish_loading_the_new_font_info() {
		...
		if (abs(tfm_design_size - d) > 2)
		{
			...
			print_ln("   (", d, " vs. ", tfm_design_size, ')');
			print("   ");
		}
		print("---loaded at size ", q, " DVI units");
		d = round((100.0 * conv * q) / (real)(true_conv * d));
		if (d != 100)
		{
			print_ln(' '); print(" (this font is magnified ", d, "%)");
		}
		//@.this font is magnified@>
		incr(nf); /*now the new font is officially present*/
		}
	}
}; inline void define_font(integer e) { define_font_()(e); }
...
@^system dependencies@>
*/
constexpr auto default_directory_name = "TeXfonts/"; /*change this to the correct name*/
constexpr auto default_directory_name_length = 9; /*change this to the correct length*/

packed_array_of_char < subrange<1, default_directory_name_length>> default_directory;
...
@^system dependencies@>
*/
void define_font_::Move_font_name_into_the_cur_name_string() {
...
*/
uint8_t text_ptr = 0; /*the number of characters in |text_buf|*/
...
*/
#include "Globals_in_the_outer_block.cpp"
integer h, v, w, x, y, hh, vv; /*current state values*/
integer hstack[stack_size + 1], vstack[stack_size + 1], wstack[stack_size + 1], xstack[stack_size + 1], ystack[stack_size + 1], zstack[stack_size + 1];
/*pushed down values in \.{DVI} units*/
integer hhstack[stack_size + 1], vvstack[stack_size + 1];
...
*/
static integer max_v = 017777777777 - 99; /*the value of |abs(v)| should probably not exceed this*/
static integer max_h = 017777777777 - 99; /*the value of |abs(h)| should probably not exceed this*/
integer max_s = stack_size + 1; /*the stack depth should not exceed this*/
integer max_v_so_far = 0, max_h_so_far = 0, max_s_so_far = 0; /*the record high levels*/
static integer total_pages; /*the stated total number of pages*/
integer page_count = 0; /*the total number of pages seen so far*/
...
opcode.
*/
#define four_cases(X)	 case X: case X+1: case X+2: case X+3
#define eight_cases(X)	 four_cases(X): four_cases(X+4)
#define sixteen_cases(X)	 eight_cases(X): eight_cases(X+8)
#define thirty_two_cases(X)	 sixteen_cases(X): sixteen_cases(X+16)
#define sixty_four_cases(X)	 thirty_two_cases(X): thirty_two_cases(X+32)

integer first_par(eight_bits o)
{
	using namespace DVI;
	switch (o) {
		sixty_four_cases(set_char_0) :
			sixty_four_cases(64 + set_char_0) :
	...
	case right1: case w1: case x1: case down1: case DVI::y1: case z1: return signed_byte(); break;
	case right1 + 1: case w1 + 1: case x1 + 1: case down1 + 1: case DVI::y1 + 1: case z1 + 1: return signed_pair(); break;
	case right1 + 2: case w1 + 2: case x1 + 2: case down1 + 2: case DVI::y1 + 2: case z1 + 2: return signed_trio(); break;
	case set1 + 3: case set_rule: case put1 + 3: case put_rule: case right1 + 3:
	case w1 + 3: case x1 + 3: case down1 + 3: case DVI::y1 + 3: case z1 + 3:
	...
	undefined_commands: return 0; break;
	case w0: return w; break;
	case x0: return x; break;
	case DVI::y0: return y; break;
	case z0: return z;
		sixty_four_cases(fnt_num_0) : return o - fnt_num_0; break;
	}
	return 0;
}
...
*/
integer s; /*current stack size*/
integer ss; /*stack size to print*/
...
*/
boolean special_cases(eight_bits o, integer p, integer a);
class do_page_ final
{
	enum goto_ { nowhere, fin_set, fin_rule, move_right, show_state, done, label9998, label9999 };
#define do_page_goto(l) goto_label_switch_(l, \
goto_label_case_(label9998); goto_label_case_(label9999); )
	eight_bits o; /*operation code of the current command*/
	integer p, q; /*parameters of the current command*/
	integer a; /*byte number of the current command*/
	integer hhh; /*|h|, rounded to the nearest pixel*/

	goto_ Translate_the_next_command_in_the_DVI_file_goto_9999_with_do_page_true_if_it_was_eop_goto_9998_if_premature_termination_is_needed();
	void Finish_a_command_that_sets_h_h_q();
	void Show_the_values_of_ss_h_v_w_x_y_z_hh_and_vv() const;
	void Translate_a_set_char_command() const;
	boolean Cases_for_horizontal_motion();
	goto_ Finish_a_command_that_either_sets_or_puts_a_rule();
	goto_ Finish_a_command_that_either_sets_or_puts_a_character();
	goto_ Cases_for_commands_nop_bop_dots_pop();
	goto_ Start_translation_of_command_o();
public: boolean operator()() {
	cur_font = invalid_font; /*set current font undefined*/
	s = 0; h = 0; v = 0; w = 0; x = 0; y = 0; z = 0; hh = 0; vv = 0;
	/*initialize the state variables*/
	while (true) do_page_goto(Translate_the_next_command_in_the_DVI_file_goto_9999_with_do_page_true_if_it_was_eop_goto_9998_if_premature_termination_is_needed());
label9998: print_ln('!'); return false;
label9999: return true;
}
}; inline boolean do_page() { return do_page_()(); }
...
*/
void flush_text();
template<typename ...Args> inline void show(integer a, Args... args) {
	flush_text(); showing = true; print(a, ": ", args...);
}
template<typename ...Args> inline void major(integer a, const char* str, Args... args) {
	if (out_mode > errors_only) show(a, str, args...);
}
template<typename ...Args> inline void minor(integer a, const char* str, Args... args) {
	if (out_mode > terse) { showing = true; print(a, ": ", str, args...); }
}
template<typename ...Args> inline void error(integer a, const char* str, Args... args) {
	if (!showing) show(a, str, args...); else print(" ", str, args...);
}

do_page_::goto_ do_page_::Translate_the_next_command_in_the_DVI_file_goto_9999_with_do_page_true_if_it_was_eop_goto_9998_if_premature_termination_is_needed() {
	{ using namespace DVI;
#define goto_moveright_or_done(l) goto_label_switch_(l, \
goto_label_case_(move_right); goto_label_case_(done); )
...
#define goto_the_appropriate_label_to_finish_the_job(l) goto_label_switch_(l, \
goto_label_case_return(label9998); goto_label_case_return(label9999); \
goto_label_case_(move_right); goto_label_case_(show_state); goto_label_case_(done); \
goto_label_case_(fin_set); goto_label_case_(fin_rule); )
	goto_the_appropriate_label_to_finish_the_job(Start_translation_of_command_o());
fin_set: { goto_moveright_or_done(Finish_a_command_that_either_sets_or_puts_a_character()); }
fin_rule: { goto_moveright_or_done(Finish_a_command_that_either_sets_or_puts_a_rule()); }
move_right: { Finish_a_command_that_sets_h_h_q(); goto done; }
show_state: { Show_the_values_of_ss_h_v_w_x_y_z_hh_and_vv(); goto done; }
...
*/
do_page_::goto_ do_page_::Start_translation_of_command_o() {
	using namespace DVI;
	if (o < set_char_0 + 128) { Translate_a_set_char_command(); return goto_::fin_set; }
	else {
		if (Cases_for_horizontal_motion()) return goto_::move_right;
#define Start_translation_goto(l) goto_label_switch_(l, \
goto_label_case_return(label9998); 	goto_label_case_return(label9999); 	\
goto_label_case_return(show_state); goto_label_case_return(done); 	) 
		Start_translation_goto(Cases_for_commands_nop_bop_dots_pop());
		switch (o) {
			four_cases(set1) : { major(a, "set", o - set1 + 1, " ", p); return goto_::fin_set; break;
			}
			four_cases(put1) : { major(a, "put", o - put1 + 1, " ", p); return goto_::fin_set; break;
			}
		case set_rule: {major(a, "setrule"); return goto_::fin_rule; break;
		}
		case put_rule: {major(a, "putrule"); return goto_::fin_rule; break;
		}
		default:if (special_cases(o, p, a)) return goto_::done; else return goto_::label9998;
		}
	} return goto_::nowhere;
}

class special_cases_  final {
	enum goto_ { nowhere, change_font, done };
#define special_cases_goto(l) goto_label_switch_(l, \
goto_label_case_(change_font); 	goto_label_case_(done); 	) 
	integer q; /*parameter of the current command*/
	integer k; /*loop index*/
	boolean bad_char; /*has a non-ASCII character code appeared in this \\{xxx}?*/
	boolean pure; /*is the command error-free?*/
	integer vvv; /*|v|, rounded to the nearest pixel*/
	void Finish_a_command_that_changes_the_current_font();
	void Finish_a_command_that_sets_v_v_p();
	void Translate_an_xxx_command();
	boolean Cases_for_vertical_motion();
	goto_ Cases_for_fonts();
	eight_bits o; integer p; integer a;
public: boolean operator()(eight_bits o, integer p, integer a) {
	using namespace DVI;
	this->o = o; this->p = p; this->a = a;
	pure = true;
	if (Cases_for_vertical_motion()) goto move_down;
	special_cases_goto(Cases_for_fonts());
	switch (o) {
		four_cases(xxx1) : { Translate_an_xxx_command(); goto done; }
	case pre: {error(a, "preamble command within a page!"); goto label9998; break;
	}
			//@.preamble command within a page@>
	case post: case post_post: {error(a, "postamble command within a page!"); goto label9998; break;
		//@.postamble command within a page@>
	}
	default: {error(a, "undefined command ", o, "!");
		goto done;
		//@.undefined command@>
	}
	}
move_down: { Finish_a_command_that_sets_v_v_p(); goto done; }
change_font: { Finish_a_command_that_changes_the_current_font(); goto done; }
label9998: pure = false;
done: return pure;
}
}; inline boolean special_cases(eight_bits o, integer p, integer a) { return special_cases_()(o, p, a); }

do_page_::goto_ do_page_::Cases_for_commands_nop_bop_dots_pop() {
	using namespace DVI;
	switch (o) {
	case nop: {minor(a, "nop"); return goto_::done; break;
	}
	case bop: {error(a, "bop occurred before eop!"); return goto_::label9998; break;
		//@.bop occurred before eop@>
	}
	case eop: {major(a, "eop");
		if (s != 0) error(a, "stack not empty at end of page (level ",
			s, ")!");
		//@.stack not empty...@>
		print_ln(' '); return goto_::label9999; break;
	}
	case push: {major(a, "push");
		if (s == max_s_so_far)
		{
			max_s_so_far = s + 1;
			if (s == max_s) error(a, "deeper than claimed in postamble!");
			//@.deeper than claimed...@>
			//@.push deeper than claimed...@>
			if (s == stack_size)
			{
				error(a, "DVItype capacity exceeded (stack size=",
					stack_size, ")"); return goto_::label9998;
			}
		}
		hstack[s] = h; vstack[s] = v; wstack[s] = w;
		xstack[s] = x; ystack[s] = y; zstack[s] = z;
		hhstack[s] = hh; vvstack[s] = vv; incr(s); ss = s - 1; return goto_::show_state; break;
	}
	case pop: {major(a, "pop");
		if (s == 0) error(a, "(illegal at level zero)!");
		else {
			decr(s); hh = hhstack[s]; vv = vvstack[s];
			h = hstack[s]; v = vstack[s]; w = wstack[s];
			x = xstack[s]; y = ystack[s]; z = zstack[s];
		}
		ss = s; return goto_::show_state; break;
	}
	} return goto_::nowhere;
}
...
*/
void out_text(ASCII_code c);
void out_space_(const char* str, integer val, integer a, integer& p, integer& q) {
	if ((p >= font_space[cur_font]) || (p <= -4 * font_space[cur_font]))
	{
		out_text(' '); hh = pixel_round(h + p);
	}
	else hh = hh + pixel_round(p);
	minor(a, str, val, " ", p); q = p;
}
#define out_space(str, val) out_space_(str, val, a, p, q); return true

boolean do_page_::Cases_for_horizontal_motion() {
	using namespace DVI;
	switch (o) {
		four_cases(right1) :{ out_space("right", o - right1 + 1); break;
		}
	case w0: four_cases(w1) : { w = p; out_space("w", o - w0); break;
	}
	case x0: four_cases(x1) : { x = p; out_space("x", o - x0); break;
	}
	} return false;
}
...
void out_text(ASCII_code c);
void flush_text();
void out_vmove_(const char* str, integer val, integer a, integer p, integer v, integer& vv) {
	if (abs(p) >= 5 * font_space[cur_font]) vv = pixel_round(v + p);
	else vv = vv + pixel_round(p);
	major(a, str, val, " ", p);
}
#define out_vmove(str, val) out_vmove_(str, val, a, p, v, vv); return true

boolean special_cases_::Cases_for_vertical_motion() {
	using namespace DVI;
	switch (o) {
		four_cases(down1) :{ out_vmove("down", o - down1 + 1); break;
		}
	case DVI::y0: four_cases(DVI::y1) : { y = p; out_vmove("y", o - DVI::y0); break;
	}
	case z0: four_cases(z1) : { z = p; out_vmove("z", o - z0); break;
	}
	} return false;
}

special_cases_::goto_ special_cases_::Cases_for_fonts() {
	using namespace DVI;
	switch (o) {
		sixty_four_cases(fnt_num_0) : { major(a, "fntnum", p);
		return goto_::change_font; break;
		}
		four_cases(fnt1) : { major(a, "fnt", o - fnt1 + 1, " ", p);
		return goto_::change_font; break;
		}
		four_cases(fnt_def1) : { major(a, "fntdef", o - fnt_def1 + 1, " ", p);
		define_font(p); return goto_::done; break;
		}
	} return goto_::nowhere;
}

void special_cases_::Translate_an_xxx_command() {
	{major(a, "xxx '"); bad_char = false;
	if (p < 0) error(a, "string of negative length!");
	...
	if (showing) print('\'');
	if (bad_char) error(a, "non-ASCII character in xxx command!");
	//@.non-ASCII character...@>
	}
}

void do_page_::Translate_a_set_char_command() const {
	{if ((o > ' ') && (o <= '~'))
	{
		out_text(p); minor(a, "setchar", p);
	}
	else major(a, "setchar", p);
	}
}

do_page_::goto_ do_page_::Finish_a_command_that_either_sets_or_puts_a_character() {
	using namespace DVI;
	...
	if (q == invalid_width)
	{
		error(a, "character ", p, " invalid in font ");
		//@.character $c$ invalid...@>
		print_font(cur_font);
		if (cur_font != invalid_font)
			print('!'); /*the invalid font has `\.!' in its name*/
	}
	if (o >= put1) return goto_::done;
	if (q == invalid_width) q = 0;
	else hh = hh + char_pixel_width(cur_font)(p);
	return goto_::move_right;
}

do_page_::goto_ do_page_::Finish_a_command_that_either_sets_or_puts_a_rule() {
	using namespace DVI;
	q = signed_quad();
	if (showing)
	{
		...
		if (out_mode > mnemonics_only)
			...
			else print(" (", rule_pixels(p), "x", rule_pixels(q), " pixels)");
	}
	if (o == put_rule) return goto_::done;
	if (showing) if (out_mode > mnemonics_only) print_ln(" ");
	hh = hh + rule_pixels(q); return goto_::move_right;
}
...
*/
constexpr auto infinity = 017777777777; /*$\infty$ (approximately)*/
constexpr auto max_drift = 2; /*we insist that abs|(hh-pixel_round(h)) <= max_drift|*/

void do_page_::Finish_a_command_that_sets_h_h_q() {
	if ((h > 0) && (q > 0)) if (h > infinity - q)
	{
		error(a, "arithmetic overflow! parameter changed from ",
			//@.arithmetic overflow...@>
			q, " to ", infinity - h);
		q = infinity - h;
	}
...
void special_cases_::Finish_a_command_that_sets_v_v_p() {
...
void do_page_::Show_the_values_of_ss_h_v_w_x_y_z_hh_and_vv() const {
	if (showing) if (out_mode > mnemonics_only)
	{
		print_ln(' ');
		print("level ", ss, ":(h=", h, ",v=", v,
			",w=", w, ",x=", x, ",y=", y, ",z=", z,
			",hh=", hh, ",vv=", vv, ')');
	}
}

void special_cases_::Finish_a_command_that_changes_the_current_font() {
	font_num[nf] = p; cur_font = 0;
	while (font_num[cur_font] != p) incr(cur_font);
	if (cur_font == nf)
	{
		cur_font = invalid_font;
		error(a, "invalid font selection: font ", p, " was never defined!");
	}
	if (showing) if (out_mode > mnemonics_only)
	{
...
*/
void scan_bop();
class skip_pages_ final {
	/*label9999*/ // end of this subroutine
	integer p; /*a parameter*/
	uint8_t k; /*command code*/
	integer down_the_drain; /*garbage*/
public: void operator()(boolean bop_seen) {
	using namespace DVI;
	showing = false;
	while (true)
	{
		...
		Skip_until_finding_eop();
		bop_seen = false;
	}
label9999:;
}

		void Skip_until_finding_eop() {
			using namespace DVI;
			do {
				if (eof(dvi_file)) bad_dvi("the file ended prematurely");
				//@.the file ended prematurely@>
				...
				case set_rule: case put_rule: down_the_drain = signed_quad(); break;
					...
					{
						down_the_drain = get_byte(); decr(p); break;
					}
				case bop: case pre: case post: case post_post: undefined_commands:
					bad_dvi("illegal command at byte ", cur_loc - 1); break;
					//@.illegal command at byte n@>
				default:do_nothing;
				}
			} while (!(k == eop));
		}
}; inline void skip_pages(boolean bop_seen) { skip_pages_()(bop_seen); }
...
*/
integer old_backpointer = -1; /*the previous |bop| command location*/
integer new_backpointer; /*the current |bop| command location*/
#include "Globals_in_the_outer_block.cpp"
...
void scan_bop()
{
	...
	if (k == post) in_postamble = true;
		...
		if (k != bop) bad_dvi("byte ", cur_loc - 1, " is not bop");
		//@.byte n is not bop@>
		new_backpointer = cur_loc - 1; incr(page_count);
		for (k = 0; k <= 9; k++) count[k] = signed_quad();
		if (signed_quad() != old_backpointer
			) print_ln("backpointer in byte ", cur_loc - 4,
				" should be ", old_backpointer, '!');
...
*/
void Find_the_postamble_working_back_from_the_end() {
	using namespace DVI;
	n = dvi_length();
	if (n < 53) bad_dvi("only ", n, " bytes long");
	//@.only n bytes long@>
	m = n - 4;
	do {
	...
	if (k != id_byte) bad_dvi("ID byte is ", k);
	//@.ID byte is wrong@>
	move_to_byte(m - 3); q = signed_quad();
	if ((q < 0) || (q > m - 33)) bad_dvi("post pointer ", q, " at byte ", m - 3);
	//@.post pointer is wrong@>
	move_to_byte(q); k = get_byte();
	if (k != post) bad_dvi("byte ", q, " is not post");
	//@.byte n is not post@>
...
*/
#include "Globals_in_the_outer_block.cpp"
integer start_loc; /*byte location of the first page to process*/
integer after_pre; /*byte location immediately following the preamble*/
...
*/
void Count_the_pages_and_move_to_the_starting_page() {
	using namespace DVI;
	...
	if (p < 0) in_postamble = true;
		...
		do {
			...
			if (p > q - 46)
				bad_dvi("page link ", p, " after byte ", q);
			//@.page link wrong...@>
			q = p; move_to_byte(q); k = get_byte();
			if (k == bop) incr(page_count);
			else bad_dvi("byte ", q, " is not bop");
			//@.byte n is not bop@>
			for (k = 0; k <= 9; k++) count[k] = signed_quad();
			p = signed_quad();
			if (start_match())
	...
	if (page_count != total_pages)
		print_ln("there are really ", page_count, " pages, not ", total_pages, '!');
...
*/
class read_postamble_ final {
	integer k; /*loop index*/
	integer p, q, m; /*general purpose registers*/
	void Make_sure_that_the_end_of_the_file_is_wellformed();
	void Process_the_font_definitions_of_the_postamble();
	void Compare_the_lust_parameters_with_the_accumulated_facts();
public: void operator()() {
	showing = false; post_loc = cur_loc - 5;
	print_ln("Postamble starts at byte ", post_loc, '.');
	...
	print("maxv=", max_v, ", maxh=", max_h);
	max_s = get_two_bytes(); total_pages = get_two_bytes();
	print_ln(", maxstackdepth=", max_s, ", totalpages=", total_pages);
	if (out_mode < the_works)
		Compare_the_lust_parameters_with_the_accumulated_facts();
	Process_the_font_definitions_of_the_postamble();
	Make_sure_that_the_end_of_the_file_is_wellformed();
}
}; inline void read_postamble() { read_postamble_()(); }
...
*/
void read_postamble_::Compare_the_lust_parameters_with_the_accumulated_facts() {
	{if (max_v + 99 < max_v_so_far)
		print_ln("warning: observed maxv was ", max_v_so_far);
	//@.warning: observed maxv...@>
	//@.observed maxv was x@>
	if (max_h + 99 < max_h_so_far)
		print_ln("warning: observed maxh was ", max_h_so_far);
	//@.warning: observed maxh...@>
	//@.observed maxh was x@>
	if (max_s < max_s_so_far)
		print_ln("warning: observed maxstackdepth was ", max_s_so_far);
...
just been read.
*/
void read_postamble_::Make_sure_that_the_end_of_the_file_is_wellformed() {
	q = signed_quad();
	if (q != post_loc)
		print_ln("bad postamble pointer in byte ", cur_loc - 4, '!');
	//@.bad postamble pointer@>
	m = get_byte();
	if (m != id_byte) print_ln("identification in byte ", cur_loc - 1,
		//@.identification...should be n@>
		" should be ", id_byte, '!');
	k = cur_loc; m = 223;
	while ((m == 223) && !eof(dvi_file)) m = get_byte();
	if (!eof(dvi_file)) bad_dvi("signature in byte ", cur_loc - 1,
		//@.signature...should be...@>
		" should be 223");
	else if (cur_loc < k + 4)
...
void read_postamble_::Process_the_font_definitions_of_the_postamble() {
	using namespace DVI;
	do {
		k = get_byte();
		if ((k >= fnt_def1) && (k < fnt_def1 + 4))
	...
	if (k != post_post)
		print_ln("byte ", cur_loc - 1, " is not postpost!");
...
*/
class main_ final {
	void Process_the_preamble();
	void Translate_up_to_max_pages_pages();
	void Compute_the_conversion_factors();
public:
	void operator()() {
		using namespace DVI;
		initialize(); /*get all variables initialized*/
		dialog(); /*set up all the options*/
		Process_the_preamble();
		if (out_mode == the_works)  /*|random_reading==true|*/
		{
			Find_the_postamble_working_back_from_the_end();
			in_postamble = true; read_postamble(); in_postamble = false;
			Count_the_pages_and_move_to_the_starting_page();
		}
		skip_pages(false);
		if (!in_postamble) Translate_up_to_max_pages_pages();
		if (out_mode < the_works)
		{
...
}; int dvitype::main() { main_()(); return EXIT_SUCCESS; }
...
*/
void main_::Process_the_preamble() {
	using namespace DVI;
	open_dvi_file();
	...
	if (p != id_byte)
		print_ln("identification in byte 1 should be ", id_byte, '!');
	//@.identification...should be n@>
	Compute_the_conversion_factors();
...
|n/(real)d| decimicrons per \.{DVI} unit, and 254000 decimicrons per inch,
and |resolution| pixels per inch. Then we have to adjust this
by the stated amount of magnification.
*/
void main_::Compute_the_conversion_factors() {
	numerator = signed_quad(); denominator = signed_quad();
	if (numerator <= 0) bad_dvi("numerator is ", numerator);
	//@.numerator is wrong@>
	if (denominator <= 0) bad_dvi("denominator is ", denominator);
	//@.denominator is wrong@>
	print_ln("numerator/denominator=", numerator, "/", denominator);
	tfm_conv = (25400000.0 / (real)numerator) * (denominator / (real)473628672) / 16.0;
	conv = (numerator / 254000.0) * (resolution / (real)denominator);
	mag = signed_quad();
	if (new_mag > 0) mag = new_mag;
	else if (mag <= 0) bad_dvi("magnification is ", mag);
	//@.magnification is wrong@>
	true_conv = conv; conv = true_conv * (mag / 1000.0);
	print("magnification=", mag, "; "); format(conv, 16, 8); print_ln(" pixels per DVI unit");
}
...
page.
*/
void main_::Translate_up_to_max_pages_pages() {
	{ while (max_pages > 0)
	{
		decr(max_pages);
		print_ln(" "); print(cur_loc - 45, ": beginning of page ");
		for (k = 0; k <= start_vals; k++)
		{
			print(count[k]);
			if (k < start_vals) print(".");
